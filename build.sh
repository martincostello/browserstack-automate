...
if [ $skipTests == 0 ]; then
    if [ "$TF_BUILD" != "" ]; then
        dotnet test ./tests/MartinCostello.BrowserStack.Automate.Tests/MartinCostello.BrowserStack.Automate.Tests.csproj --output $artifacts --configuration $configuration --framework netcoreapp3.1 --logger trx || exit 1
    else
        dotnet test ./tests/MartinCostello.BrowserStack.Automate.Tests/MartinCostello.BrowserStack.Automate.Tests.csproj --output $artifacts --configuration $configuration --framework netcoreapp3.1 || exit 1
    fi
fi
