...
- Some endpoints deserialize through internal transport wrappers rather than directly into the public models. For example, build and session list/detail endpoints use internal wrapper types such as `AutomationBuild`, `AutomationSession`, and `AutomationSessionDetail` to unwrap nested API payloads, while the project detail endpoint returns the public `ProjectDetailItem` model (with its nested `ProjectDetail? Project` property).
